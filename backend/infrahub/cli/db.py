...
async def trigger_rebase_branches(db: InfrahubDatabase) -> None:
    ...
    for branch in branches:
        ...
        get_migration_console().log(f"Rebasing branch '{branch.name}' (ID: {branch.uuid})...", end="")
        try:
...
RETURN vertices, edges
    ...
    with vertex_path.open(mode="w") as vertex_file, edge_path.open(mode="w") as edge_file:
        ...
        while has_more_data:
            get_migration_console().log("Retrieving batch of vertices and edges...", end="")
            results = await db.execute_query(
                query=query,
                params={"kinds": kinds, "uuids": uuids, "limit": limit, "offset": offset},
            )
            get_migration_console().log("done. ", end="")
            has_more_data = len(results) >= limit
            offset += limit

            get_migration_console().log("Writing batch to export files...", end="")
...
SET v = vertex
    ...
    get_migration_console().log(f"Loading {len(vertex_dicts)} {vertex_labels} nodes...", end="")
...
SET e = edge.properties
    """ % {"edge_type": edge_type}
    get_migration_console().log(f"Loading {len(edge_dicts)} {edge_type} edges...", end="")
